...
template<typename C>
void perform_checks(C& collisions)
{
    ...
    CHECK(not collisions.contains(collisions.all_without_joints(), 2));
    CHECK(not collisions.contains(collisions.all_without_joints(), 4));
    CHECK(collisions.contains(collisions.all_without_joints(), 1));
    CHECK(collisions.contains(collisions.all_without_joints(), 3));
...
TEST_CASE("container2_set")
{
    detail::ContainsInfo::reset();

    auto collisions = Collisions2<std::set>{};
...
TEST_CASE("container2_bag")
{
    detail::ContainsInfo::reset();

    auto collisions = Collisions2<CollisionBag>{};
